...
def parse_opt():
    parser = argparse.ArgumentParser(
        description="Train a custom object detection model."
    )
    parser.add_argument(
        "--epochs",
        type=int,
        default=10,
        help="Number of epochs for phase 1 (feature extraction).",
    )
    parser.add_argument(
        "--fine-tune-epochs",
        type=int,
    ...
    parser.add_argument(
        "--learning-rate", type=float, default=0.0001, help="Learning rate for phase 1."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--load-weights",
        action="store_true",
        help="Load existing model weights if available and continue training.",
    )
    parser.add_argument(
        "--export-model",
        action="store_true",
        help="Export the trained model to TensorFlow.js format.",
    )
...
class CorrectDetectionRate(tf.keras.metrics.Metric):
    ...
    def _calculate_iou(self, y_true, y_pred):
        ...
        inter_area = tf.maximum(0.0, inter_x2 - inter_x1) * tf.maximum(
...
def create_custom_object_detection_model():
    ...
    """
    input_tensor = Input(
        shape=(IMG_HEIGHT, IMG_WIDTH, IMG_CHANNELS), name="input_image"
    )
    ...
    transformer_block = TransformerEncoder(
        embed_dim=c, num_heads=4, dense_dim=c, name="transformer_encoder"
    )
    ...
    model = Model(
        inputs=input_tensor, outputs=bbox_output, name="custom_object_detector"
    )
...
def train_model():
    ...
    if opt.load_weights and os.path.exists(save_path):
        ...
        model = tf.keras.models.load_model(
            str(save_path),
            custom_objects={
                "CorrectDetectionRate": CorrectDetectionRate,
    ...
    weighted_mse = create_weighted_mse_loss(
        location_weight=2.5
    )  # Emphasize location accuracy
    ...
    model.compile(
        optimizer=tf.keras.optimizers.Adam(learning_rate=opt.fine_tune_learning_rate),
        loss=weighted_mse,
        metrics=["mae", CorrectDetectionRate()],
    )
