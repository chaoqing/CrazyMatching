...
def evaluate_model(
    model, dataset, device, score_threshold=0.5, iou_threshold=0.5, num_visualizations=5
):
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Evaluate a trained SSD object detection model."
    )
    parser.add_argument(
        "--model_path",
        type=str,
    ...
    if not os.path.exists(model_full_path):
        print(
            f"Error: Model weights not found at {model_full_path}. Please train the model first."
        )
    elif not os.path.exists(
        os.path.join(data_full_path, "images")
    ) or not os.path.exists(os.path.join(data_full_path, "annotations")):
        ...
        dataset = PascalVOCDataset(
            os.path.join(data_full_path), get_transform(train=False)
        )
        data_loader = DataLoader(
            dataset,
            batch_size=1,
            shuffle=False,
            num_workers=0,  # Set num_workers to 0 for Windows compatibility or debugging
