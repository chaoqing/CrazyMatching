...
def train_model(
    ...
    eval_dataset = PascalVOCDataset(
        os.path.join(data_root), get_transform(train=False), train=False
    )
    ...
    for epoch in range(num_epochs):
        ...
        lr_scheduler.step()
        print(
            f"Epoch {epoch + 1} finished. Average Loss: {total_loss / len(data_loader):.4f}"
        )

        if epoch % 5 == 0:
            # Evaluation phase using the new evaluate_model function
            avg_iou = evaluate_model(
                model, eval_dataset, device, num_visualizations=0
            )  # Set num_visualizations to 0 during training
    ...
    with torch.no_grad():
        ...
        dataset_eval = PascalVOCDataset(
            os.path.join(data_root), get_transform(train=False)
        )
        if len(dataset_eval) > 0:
            img_pil, _ = dataset_eval[0]  # Get PIL image and original target
            img_tensor = (
                get_transform(train=False)(img_pil, {})[0].unsqueeze(0).to(device)
            )
            prediction = model(img_tensor)[0]

            # Convert tensor to numpy for plotting
            img_np = (
                img_pil.permute(1, 2, 0).cpu().numpy()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data_root",
        type=str,
        default="trainning_data",
        help="Root directory for dataset.",
    )
    parser.add_argument(
        "--save_path",
        type=str,
        default="./ssd_model.pth",
        help="Path to save the trained model.",
    )
    parser.add_argument(
        "--export_onnx",
        action="store_true",
        help="Export the trained model to ONNX format.",
    )
    parser.add_argument(
        "--onnx_output_path",
        type=str,
        default="./ssd_model.onnx",
    ...
    if not os.path.exists(os.path.join(data_output_dir, "images")):
        print(
            f"Data not found in {data_output_dir}. Generating {args.num_samples} samples..."
        )
        print(
            f"uv run python {data_gen_script} --num_samples {args.num_samples} --output_base_dir {data_output_dir}"
        )
        exit(1)
    ...
    if args.export_onnx:
        print(
            f"Exporting model to ONNX: {os.path.join('./torch_ssd', args.onnx_output_path)}"
        )
        device = torch.device("cpu")
        model = create_ssd_model(num_classes=len(CLASSES), pretrained=True)
        model.load_state_dict(
            torch.load(os.path.join("./torch_ssd", args.save_path), map_location=device)
        )
        model.to(device)
        model.eval()
